...
Load balancing ensures that the application will be highly available in addition to restricting overload to the network.  It also plays a critical role in the redundancy of our setup because it forwards the traffic to the redundant virtual machines, specifically Web-2 and Web-3.  As a result, these extra machines cannot do their job without receiving the web traffic that comes from a properly configured and a properly working load balancer.  And when they are connected to a properly configured and working load balancer, redundancy has its intended effect and the application will be highly available, reliable, and efficient.           
